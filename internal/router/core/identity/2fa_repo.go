...
type TwoFARepo struct {
	*gormutil.BaseRepo[CoreTwoFA]
}

// NewTwoFARepo 创建 2FA 数据访问层实例
func NewTwoFARepo(db *gorm.DB) *TwoFARepo {
	return &TwoFARepo{BaseRepo: gormutil.NewBaseRepo[CoreTwoFA](db)}
}

// GetTwoFAByUserID 根据用户 ID 获取 2FA 配置
func (r *TwoFARepo) GetTwoFAByUserID(ctx context.Context, userID string) (*CoreTwoFA, error) {
	var twoFA CoreTwoFA
...
func (r *TwoFARepo) CreateTwoFA(ctx context.Context, twoFA *CoreTwoFA) error {
	return r.Create(ctx, twoFA)
}

// UpdateTwoFA 更新 2FA 配置
func (r *TwoFARepo) UpdateTwoFA(ctx context.Context, twoFA *CoreTwoFA) error {
	return r.Update(ctx, twoFA)
}

// DeleteTwoFA 删除 2FA 配置
func (r *TwoFARepo) DeleteTwoFA(ctx context.Context, userID string) error {
	return r.GetDB(ctx).Where("user_id = ?", userID).Delete(&CoreTwoFA{}).Error
}

// CreateTwoFALog 创建 2FA 验证日志
func (r *TwoFARepo) CreateTwoFALog(ctx context.Context, log *CoreTwoFALog) error {
