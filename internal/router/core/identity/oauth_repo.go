...
func (r *OAuthRepo) CreateOAuthCode(ctx context.Context, code *OAuthCode) error {
	return r.GetDB(ctx).Create(code).Error
}

// GetOAuthCodeByCode 根据授权码获取 OAuth 授权码
func (r *OAuthRepo) GetOAuthCodeByCode(ctx context.Context, code string) (*OAuthCode, error) {
...
func (r *OAuthRepo) DeleteOAuthCode(ctx context.Context, code string) error {
	return r.GetDB(ctx).Where("code = ?", code).Delete(&OAuthCode{}).Error
}
...
func (r *OAuthRepo) DeleteExpiredOAuthCodes(ctx context.Context) error {
	return r.GetDB(ctx).Where("expires_at < NOW()").Delete(&OAuthCode{}).Error
}
